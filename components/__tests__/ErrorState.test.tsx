...
describe('ErrorState', () => {
  it('maps a known error.message to its French line', () => {
    renderWithIntl(<ErrorState error={new Error('Unauthorized')} reset={vi.fn()} home={home} />)
  ...
  it('falls back to the generic French line for an unknown message', () => {
    renderWithIntl(<ErrorState error={new Error('boom')} reset={vi.fn()} home={home} />)
  ...
  it('renders the home link with the passed href + label and calls reset', () => {
    const reset = vi.fn()
    renderWithIntl(<ErrorState error={new Error('boom')} reset={reset} home={home} />)
  ...
  it('student boundary wires home to the dossier', () => {
    renderWithIntl(<StudentError error={new Error('boom')} reset={vi.fn()} />)
