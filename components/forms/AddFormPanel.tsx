  ...
  return (
    <div className="mb-4 rounded-[14px] border border-dashed border-frame bg-hoverrow p-[18px]">
      <div className="mb-3.5 flex items-center justify-between">
        <div className="font-mono text-[10px] font-semibold uppercase tracking-[.1em] text-tertiary">{t('forms.addFormLabel')}</div>
        <button type="button" onClick={onClose} aria-label={t('forms.close')} className="h-[26px] w-[26px] rounded-[7px] border bg-card text-[13px] text-tertiary">✕</button>
      </div>
      ...
      {mode === null ? (
        <div className="grid grid-cols-2 gap-3.5">
          <button type="button" onClick={() => setMode('pdf')} className="flex flex-col items-start gap-2.5 rounded-xl border bg-card p-[18px] text-left hover:border-brand">
            ...
            <div className="font-display text-[15px] font-semibold text-navy">{t('forms.addPanel.pdfTile.title')}</div>
            <div className="text-[12.5px] leading-normal text-muted-foreground">{t('forms.addPanel.pdfTile.description')}</div>
            <span className="mt-0.5 rounded-lg bg-subtle px-3.5 py-2 text-[12.5px] font-semibold text-navy">{t('forms.addPanel.pdfTile.button')}</span>
          </button>
          <button type="button" onClick={() => setMode('online')} className="flex flex-col items-start gap-2.5 rounded-xl border bg-card p-[18px] text-left hover:border-brand">
            ...
            <div className="font-display text-[15px] font-semibold text-navy">{t('forms.addPanel.onlineTile.title')}</div>
            <div className="text-[12.5px] leading-normal text-muted-foreground">{t('forms.addPanel.onlineTile.description')}</div>
            <span className="mt-0.5 rounded-lg bg-brand px-3.5 py-2 text-[12.5px] font-semibold text-white">{t('forms.addPanel.onlineTile.button')}</span>
          </button>
        </div>
      ) : (
        <form onSubmit={handleCreate} className="flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-3.5">
            <div className="flex flex-col gap-1">
              <label htmlFor="add-form-name" className="text-[13px] font-semibold text-navy">{t('forms.addPanel.nameLabel')}</label>
              <input id="add-form-name" value={name} onChange={(e) => setName(e.target.value)} required
                placeholder={mode === 'pdf' ? t('forms.addPanel.namePlaceholderPdf') : t('forms.addPanel.namePlaceholderOnline')}
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="add-form-deadline" className="text-[13px] font-semibold text-navy">{t('forms.addPanel.deadlineLabel')}</label>
          ...
          {mode === 'pdf' && (
            <div className="flex flex-col gap-1">
              <label htmlFor="add-form-file" className="text-[13px] font-semibold text-navy">{t('forms.addPanel.fileLabel')}</label>
          ...
          <div className="flex gap-2.5">
            <button type="submit" disabled={busy} className="rounded-[9px] bg-brand px-4 py-2.5 text-[13px] font-semibold text-white hover:bg-brand-hover disabled:opacity-60">
              {busy ? t('forms.addPanel.creating') : t('forms.addPanel.createDraft')}
            </button>
            <button type="button" onClick={() => setMode(null)} className="rounded-[9px] px-4 py-2.5 text-[13px] font-semibold text-muted-foreground">{t('forms.addPanel.back')}</button>
          </div>
        </form>
      )}
    </div>
