...
- **Never type a helper's translator param as `ReturnType<typeof useTranslations>`** — it explodes into TS2590 "union type too complex", which vitest does not catch and `tsc` does. Shared helpers take `AppTranslator` (Task 2) and React call sites wrap their `t` with `asAppTranslator(t)`; next-intl's literal key union is not directly assignable. Verified against `tsc` before any consumer was written.
- **Edit `messages/*.json` with targeted text edits, never a `json.load`/`json.dump` round-trip** — that reflows unrelated compact objects into multi-line noise. When matching a generic key like `"tabs"`, scope the match by a sibling key, since the same leaf name recurs across namespaces.
