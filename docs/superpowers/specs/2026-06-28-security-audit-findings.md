...
| H3 | ◑ Final step left | Resend key rotated. App + edge function + cron migrated to the new `sb_publishable_`/`sb_secret_` keys (the exposed `service_role` is a legacy key). **Final step:** deactivate the legacy API keys in the dashboard to invalidate the exposed key. |
| M2 | ✅ Fixed | `send-reminders` deployed with `verify_jwt=false` + a `CRON_SECRET` gate, decoupled from the legacy key via an explicit `SERVICE_KEY` (new `sb_secret_…`). Verified live: 401 with no/wrong secret, 200 + `{students:0,emailsSent:0}` with the correct secret. Cron re-pointed to send `x-cron-secret` (no legacy bearer). |
