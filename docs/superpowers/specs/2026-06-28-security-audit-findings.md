...
| H3 | ✅ Fixed | Resend key rotated; app + edge function + cron migrated to new `sb_publishable_`/`sb_secret_` keys; **legacy API keys deactivated** → the exposed `service_role` is invalidated. Edge function verified 200 on `SERVICE_KEY` post-deactivation. |
