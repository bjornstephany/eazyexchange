...
- Migrate the remaining native `<input type="date">` fields (LibraryDrawer, template editors) to `components/ui/date-field` — the Candidatures deadlines moved 2026-07-29. (`app/onboarding/OnboardingForm.tsx`'s trip start/end used to be on this list too; `/onboarding` was removed 2026-08-13 and the module is parked, unreachable and deliberately untouched, so it no longer needs migrating — drop it if the module is ever deleted for good.)
...
- `DateField`'s day cells use the `aria-pressed` toggle-button pattern for selection rather than the APG date-picker `role="grid"`/`gridcell` + `aria-selected` pattern — correct-and-valid but imperfect; revisit alongside the keyboard-navigation line above.
...
- [2026-07-25] revoke-schools-name-grant — `authenticated` has no UPDATE on
  ...
  policy dropped with it. `claim_school()` was the only writer of the name at
  the time; `/onboarding` (its one caller) was removed 2026-08-13, so nothing
  writes `schools.name` any more and `claim_school()` survives as an RPC with
  no caller.
